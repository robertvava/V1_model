...
void pushMergedNeuronInitGroup0ToDevice(unsigned int idx, unsigned int* spkCnt, unsigned int* spk, scalar* V, scalar* RefracTime, float* inSynInSyn0, float* inSynInSyn1, unsigned int numNeurons) {
    MergedNeuronInitGroup0 group = {spkCnt, spk, V, RefracTime, inSynInSyn0, inSynInSyn1, numNeurons, };
...
// merged extra global parameter functions
// ------------------------------------------------------------------------
__device__ unsigned int d_mergedNeuronInitGroupStartID0[] = {0, 3200, };
__device__ unsigned int d_mergedSynapseConnectivityInitGroupStartID0[] = {4000, 7200, };
__device__ unsigned int d_mergedSynapseConnectivityInitGroupStartID1[] = {8000, 11200, };
            ...
            group->spk[lid] = 0;
             {
                ...
                initVal = (-6.00000000000000000e+01f) + (curand_uniform(&localRNG) * scale);
                group->V[lid] = initVal;
            }
             {
                scalar initVal;
